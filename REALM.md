...
## ⚠ Calling convention — confirmed `__fastcall`
...
case) byte-for-byte. `realm.zig` stages `findPlayerTokenImpl` + its shim;
`enableTokenValidation()` registers it (not called yet — handler returns 0/reject
until the out-params + D2CS check are implemented).

## What d2gs still needs
1. Implement the callback table (Zig) bridging to the user's D2CS/D2DBS, then
   call `SetupAsBnetServer(&table)` during bootstrap.
2. Likely call `QSERVER_SetGlobalInstance(state, cookie)` @0x0052c0a0 with a valid
   `D2QServerGameStateStrc` before realm traffic (it HALTs on null) — confirm the
   state struct / where the host allocates it (VERIFY.md #2).
3. Decode the exact `0xFF` subtype semantics + the D2CS↔GS framing to match the
   user's existing realm.
