...
pub fn registerGame(name: []const u8, gameid: u32, gs_ip: [4]u8, gs_port: u16, gsid: u32, players: u16, status: u8, difficulty: u8, password: []const u8, description: []const u8, ttl_s: u32) bool {
    ...
    const hlen = ttlHeader(&vb, ttl_s);
    // Fields: gameid ip port gsid players status difficulty <password> <description>. Every field up to the
    ...
    const body = std.fmt.bufPrint(vb[hlen..], "{d} {d}.{d}.{d}.{d} {d} {d} {d} {d} {d} {s} {s}", .{ gameid, gs_ip[0], gs_ip[1], gs_ip[2], gs_ip[3], gs_port, gsid, players, status, difficulty, password, description }) catch return false;
    fs_lock.lock();
    defer fs_lock.unlock();
...
pub fn setGamePlayers(gameid: u32, players: u16) bool {
    ...
    const body = std.fmt.bufPrint(ob[hlen..], "{d} {d}.{d}.{d}.{d} {d} {d} {d} {d} {d} {s} {s}", .{
