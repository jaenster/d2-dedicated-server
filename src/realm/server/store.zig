...
pub fn registerGame(name: []const u8, gameid: u32, gs_ip: [4]u8, gs_port: u16, gsid: u32, players: u16, status: u8, password: []const u8, description: []const u8) bool {
    return switch (ephemeral) {
        .fs => fs.registerGame(name, gameid, gs_ip, gs_port, gsid, players, status, password, description, game_ttl_s),
        .redis => redis.registerGame(name, gameid, gs_ip, gs_port, gsid, players, status, password, description, game_ttl_s),
        .pg => pg.registerGame(name, gameid, gs_ip, gs_port, gsid, players, status, password, description, game_ttl_s),
    };
}
